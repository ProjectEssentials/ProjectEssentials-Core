...
internal object WorldBorderCommand : VanillaCommandBase() {
    ...
    private var aliases =
        configuration.take().aliases.worldborder + "worldborder"

    private fun tryAssignAliases() {
        if (!ModuleAPI.isModuleExist("cooldown")) return
        CommandAliases.aliases["worldborder"] = aliases.toMutableList()
    }

    fun register(dispatcher: CommandDispatcher<CommandSource>) {
        CommandAPI.removeCommand("worldborder")
        tryAssignAliases()
    ...
    private fun checkPermissions(source: CommandSource) {
        try {
            if (!hasPermission(source.asPlayer(), "native.worldborder", 2)) {
