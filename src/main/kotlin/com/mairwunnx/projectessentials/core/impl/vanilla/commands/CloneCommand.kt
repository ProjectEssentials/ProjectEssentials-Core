...
internal object CloneCommand : VanillaCommandBase() {
    ...
    private var aliases =
        configuration.take().aliases.clone + "clone"

    private fun tryAssignAliases() {
        CommandAliases.aliases["clone"] = aliases.toMutableList()
    }

    fun register(dispatcher: CommandDispatcher<CommandSource>) {
        CommandAPI.removeCommand("clone")
        tryAssignAliases()
    ...
    private fun checkPermissions(source: CommandSource) {
        try {
            if (!hasPermission(source.asPlayer(), "native.clone", 2)) {
                            ...
                            for (l in mutableboundingbox.minX..mutableboundingbox.maxX) {
                                ...
                                if (filterPredicate.test(cachedblockinfo)) {
                                    ...
                                    if (tileentity != null) {
                                        ...
                                        list1.add(BlockInfo(blockpos3, blockstate, compoundnbt))
