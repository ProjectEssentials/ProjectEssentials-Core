...
internal object DataPackCommand : VanillaCommandBase() {
    ...
    private var aliases =
        configuration.take().aliases.datapack + "datapack"

    private fun tryAssignAliases() {
        if (!ModuleAPI.isModuleExist("cooldown")) return
        CommandAliases.aliases["datapack"] = aliases.toMutableList()
    }

    fun register(dispatcher: CommandDispatcher<CommandSource>) {
        CommandAPI.removeCommand("datapack")
        tryAssignAliases()
    ...
    private fun checkPermissions(source: CommandSource) {
        try {
            if (!hasPermission(source.asPlayer(), "native.datapack", 2)) {
