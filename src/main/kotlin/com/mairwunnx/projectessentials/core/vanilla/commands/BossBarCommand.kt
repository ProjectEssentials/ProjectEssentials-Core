...
internal object BossBarCommand {
    ...
    private var aliases =
        CommandsConfigurationUtils.getConfig().aliases.bossbar + "bossbar"

    private fun tryAssignAliases() {
        if (!EntryPoint.cooldownInstalled) return
        CommandsAliases.aliases["bossbar"] = aliases.toMutableList()
    }
                    ...
                    Commands.literal("set").then(
                        Commands.argument(
                            ...
                            Commands.literal("players").executes { p_201430_0_ ->
                                setPlayers(p_201430_0_.source, getBossbar(p_201430_0_), emptyList())
                            }.then(
                                Commands.argument(
                                    "targets",
    ...
    private fun checkPermissions(source: CommandSource) {
        try {
            if (!EntryPoint.hasPermission(source.asPlayer(), "native.bossbar", 2)) {
                logger.info(
                    PERMISSION_LEVEL
